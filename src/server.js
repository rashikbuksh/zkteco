...
const devicePinCounter = new Map(); // sn -> next PIN counter for reliable auto-generation
      ...
      if (items.type === 'USER') {
        ...
        if (userPin && detectedKey) {
          ...
          if (!umap.has(userPin)) {
            ...
            // Update PIN counter to ensure future auto-generated PINs don't conflict
            const pinNum = Number(userPin);
            if (!isNaN(pinNum)) {
              const currentCounter = devicePinCounter.get(sn) || 1;
              devicePinCounter.set(sn, Math.max(currentCounter, pinNum + 1));
            }
...
app.get('/api/debug/users-map', (req, res) => {
  ...
  for (const [sn, umap] of usersByDevice.entries()) {
    result[sn] = {
      count: umap.size,
      pinCounter: devicePinCounter.get(sn) || 'not set',
  ...
  res.json({
    ...
    pinCounters: Object.fromEntries(devicePinCounter.entries()),
  });
});
...
app.post('/api/device/add-user', async (req, res) => {
  ...
  const usersPin = await getNextAvailablePin(sn);

  // Auto-generate PIN if not provided
  const pinVal = pin ? clean(pin) : String(usersPin);
  ...
  const base = `C:1:DATA ADD USERINFO ${join(baseParts)}`;
  const commands = [base];
...
app.get('/api/device/next-pin', async (req, res) => {
  ...
  const nextPin = await getNextAvailablePin(sn, startPin);
...
async function getNextAvailablePin(sn, startPin = 1) {
  // Get existing user map (will queue fetch if empty, but return immediately)
  const umap = await ensureUsersFetched(sn);
  let pin = Number(startPin) || 1;

  // Get or initialize the PIN counter for this device
  if (!devicePinCounter.has(sn)) {
    // Initialize counter based on existing users or default
    ...
    if (existingPins.length > 0) {
      ...
      devicePinCounter.set(sn, maxPin + 1);
    } else {
      devicePinCounter.set(sn, pin);
    }
  }

  // Get the current counter value
  let currentCounter = devicePinCounter.get(sn);

  // Ensure we start from at least the startPin
  currentCounter = Math.max(currentCounter, pin);

  // Find next available PIN (in case there are gaps or conflicts)
  while (umap.has(String(currentCounter))) {
    currentCounter++;
  }

  // Update the counter for next time
  devicePinCounter.set(sn, currentCounter + 1);
...
app.post('/api/device/bulk-add-users', async (req, res) => {
  ...
  let currentPin = await getNextAvailablePin(sn, startPin);
  const commands = [];
